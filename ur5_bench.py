...
def makeRobotsNaive(robots, bases):
     naive_pose = arr([0,0, -np.pi/2, 0, 0, 0, 0])
...
def testpart(trajs, bases, yaws, T_max):
     ...
     for ind, traj in enumerate(trajs):
          ...
          for i in range(len(traj)-1):
               p.addUserDebugLine(list(traj[i]), list(traj[i+1]), lineColorRGB=[.9, 1-0.12*ind, 0.25*ind], lineWidth=4, lifeTime = 10)
          ...
          try:
               ...
               for idx, robot in enumerate(robots):
                    ...
                    if robot_class.deadlock:
                         ...
                         applySelfPF(robot, robot_class, self_crit_nums)
...
def simulationRobot(robot_id, joint_target):
     ...
     joint_target = [joint_target[q-1] for q in joints]
     Pscale = 0.03
     Dscale = 0.2
...
def applySelfPF(robot, robot_class, crit_nums):
     ...
     joints = list(range(6))#getRevoluteJointList(robot)
...
def main():
     ...
     while curr_iteration < total_iteration:
          ...
          # print(trajs)
          # print(bases)
          success = testpart(trajs, bases, yaws, T_max)
